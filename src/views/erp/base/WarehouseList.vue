<template>
  <div class="warehouse-list">
    ...
    <div class="warehouse-list__table-wrap">
      <CommonTable
        v-loading="loading"
        :columns="columns"
        :data="tableData"
      ...
      >
        <template #column-type="{ row }">
          <el-tag
            :type="warehouseTypeTagTypeMap[(row as Warehouse).type]"
            effect="light"
            size="small"
          >
            {{ (row as Warehouse).typeLabel }}
          </el-tag>
        </template>

        <template #column-status="{ row }">
          <el-tag
            :type="statusTagTypeMap[(row as Warehouse).status]"
            effect="light"
            size="small"
          >
            {{ statusLabelMap[(row as Warehouse).status] }}
          </el-tag>
        </template>
        ...
        <template #operation="{ row }">
          ...
          <el-button
            :type="(row as Warehouse).status === 'enabled' ? 'warning' : 'success'"
            link
            size="small"
            @click="handleToggleStatus(row)"
          >
            {{ (row as Warehouse).status === 'enabled' ? '停用' : '启用' }}
          </el-button>
        </template>
      </CommonTable>
    </div>
...
const columns: TableColumn[] = [
  ...
  { prop: 'type', label: '仓库类型', width: 100, align: 'center', slot: true },
  { prop: 'address', label: '仓库地址', minWidth: 200 },
...
const handleToggleStatus = async (row: Warehouse) => {
  ...
  try {
    await ElMessageBox.confirm(`确定${actionText}仓库「${row.name}」吗？`, `${actionText}确认`, {
