...
const dialogTitle = computed(() => {
  ...
  return titles[props.mode] || '代理商退货单'
})
...
const calcAmount = (row: AgentReturnItem) => {
  ...
  (val) => {
    if (val) {
      if (props.mode !== 'add' && props.record) {
        ...
        formData.value = {
          ...defaultFormData(),
          returnNo: `AR-${new Date().getFullYear()}-${String(Date.now()).slice(-4)}`,
        }
      }
    }
  }
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (props.mode === 'add') {
        await addAgentReturn(payload)
      } else if (props.record) {
        await updateAgentReturn(props.record.id, payload)
      }
