...
**Vue 3 + Vite**：Composition API、`<script setup>`、极速冷启动与 HMR
**Element Plus**：按组件手动注册 + 按需样式，打包体积更小
**动态路由 + 后端菜单驱动**：登录后根据后端返回的功能树动态生成菜单与路由
**按钮级权限**：基于 `function_code:action` 的细粒度 `has()` 权限判断
**Pinia 持久化**：用户登录态、Tabs 状态自动持久化到 `localStorage`
**Mock 数据一键切换**：开发环境本地 Mock，生产构建自动树摇剔除
**多环境支持**：`development / test / production` 三套构建与预览脚本
**Axios 封装**：统一的 `Result<T>` 响应结构、拦截器错误提示、Token 自动注入
